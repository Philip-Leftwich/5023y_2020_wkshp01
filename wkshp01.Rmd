...
```{r welcome, fig.margin = TRUE, echo = FALSE, fig.width=15}
knitr::include_graphics("img/r_first_then.png")
```

- **Make sure you read this whole document now, and complete all the solo instructions before our group session on Friday.**
- **At the end of each week is a Blackboard quiz for you to complete - these have a deadline of midday Monday**
...
The main goal of this week is to introduce you to R and RStudio^[[R](https://www.r-project.org/). R is the name of the programming language itself and [RStudio](img/rstudio-ide.pdf) is a convenient interface.], which we will be using throughout the course in order to learn statistical concepts, bioinformatics & data visualisation.^[**Pro-tip**: these side-bars are full of extra info & hyperlinks to extra information!]

Eventually we will also add extra tools like GitHub^[[GitHub](https://www.youtube.com/watch?v=w3jLJU7DT5E) and Markdown which is a fantastic resource for data reproducibility and collaborative programming, check out this short (and very cheesy) intro video.], which are collaboration and version control systems that we will be using throughout the course. More on this in future weeks.

By the end of this module I hope you will have the tools to comfidently analyze real data, make informative and beautifual data visuals, and be able to analyse lots of different types of data.

The taught content this autumn will be given to you in several **worksheets**, of which this is the first. Each worksheet covers one **topic** within the overall **module** and will be split into **sections**. You should aim to complete *one section per week*. I will send you regular emails to remind you about this. Each section will comprise of a mini-lecture, some solo and group exercises and suggestions for further reading. 

Our first topic "Hello R!" is only one week long, but in the future topics will run over several weeks.


# Week One

We have **two** live events in week 1^[In follow on weeks, our Tuesday lecture time will be 'flipped' a place to ask questions and review the previous week. Each week there will be short assignments and projects, some will be designed to completed solo, while others are designed to be collaborative and we can run in our Friday session]: 

- Tuesday 2PM (1 hour) - Introduction and questions about the course. I will give an overview of the course and introduce myself. And we will run a simple Q&A.

- Friday 2PM/3PM depending on your group (1 hour) - Introducing yourself to R. Everything you need to complete this week's work is already in this worksheet, but on Friday we will host a live Blackboard Collaborate session to go through this together. I have decided to split the class into two groups (A&B)^[Check Blackboard for your group - this doesn't show up on e-vision yet so make sure **you** know when to show up] so that these sessions are shorter, but so are the groups and we can interact more easily.    
...
## 1.1 Aspirations
We are going to start somewhere basic like this...
```{r examples}
x <- 1+1
x
```

And by the end of this course we will be able to do things like making beautiful graphics and fitting explanatory models to them
...
or process complex sequencing data and building microbiome community analyses

```{r fastq image, fig.margin = TRUE, fig.width=3, echo=FALSE, fig.cap="Raw FASTQ data from an Illumina MiSeq platform"}
knitr::include_graphics("img/fastq.png")
```

```{r microbiome figure, echo = FALSE, warning=FALSE, message=FALSE}
knitr::include_graphics("img/Fig1 200520.png")
```
...
### Navigating RStudio
<div style="position:relative;height:0;padding-bottom:40%"><iframe width="560" height="315" src="https://www.youtube.com/embed/8mWPwPBnAsY" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>


### RStudio Cloud
<div style="position:relative;height:0;padding-bottom:40%"><iframe width="560" height="315" src="https://www.youtube.com/embed/qoAKiru4_R8" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>

```{r error figure, echo = FALSE, warning=FALSE, message=FALSE, fig.margin = TRUE}
knitr::include_graphics("img/Error.jpg")
```
## Error!

**Here are some of the most common error messages you’ll meet when you get started, and what they actually mean:**

`Error in function-name(...): could not find function ‘function-name’ :` This probably means that you are trying to use a function from a package you have not loaded. Type ??function-name (or look it up online) to find out which package it comes from, and make sure the package is loaded using library(package-name). Which leads us to…

`Error in library(package-name): there is no package called ‘package-name’.` : Did you install the package? Packages need to be installed once, using install.packages('packag-name'), after which they can be loaded with the library() command.

`Error in function-name(): object ‘object-name’ not found:` R tells you that it cannot find an object that should be in your environment. First, make sure that you have an object with this name in your Environment panel. It could be just a typo, for instance you defined the object as birds_nests yet you try to plot it calling Bird_nests. (And yes, R is also case-sensitive!) Or it could be that you haven’t actually run the code which creates this object yet!

`Error: unexpected symbol in ‘line-of-code’ :` Most likely you forgot (or had an extra!) comma, bracket or other punctuation sign somewhere.

`Cannot open file ‘your-file-path’: No such file or directory:` R cannot find the file you are trying to load, often because you forgot to specify your working directory (we learned how to do it in our first tutorial, or because there is a mistake in your file path. (Hint: if you copied it from the Windows Explorer, you need to swap the backward slashes for forward slashes.) If you get object ‘your-file-path’ not found, then you probably forgot the quotation marks around the file path read.csv(file = 'my-file-path/file.csv').

### Warning
Errors are the scary red text, but often you might get `warnings` instead. these happen when the code has run successfully, but R wants you to be aware of something. Don't ignore these. It could be a specific warning telling you that R is ignoring some data in a plot (if it falls outside of the axis limits, or doesn't have a value), or that a function gave an unexpected result. You should make sure you understand what R is trying to warn you about and decide if something needs correcting

```{r no-one flies solo, echo = FALSE, fig.height=15, fig.margin = TRUE}
knitr::include_graphics("img/code_hero.jpg")
```
## Help! 
Practice is the best way to get used to R, and make it less scary. 

-  **Google**: make sure you put the whole error message in.

-  Check your scripts against recipes e.g. Read [R4DS](https://r4ds.had.co.nz/)

-  If you can't solve a problem, post in on our discussion board (Remember to post code and warnings). **Even** better if you *had* a problem but solved it, post there too! If you see problems posted you think you can solve - have a go. We work best as a community! 

As we progress you can try asking online communites for help too:

-  [Stack Overflow](https://t.co/E5HO42BcfH?amp=1)

-  [RStudio Community](https://t.co/AVbShXNerG?amp=1)

-  [Twitter](https://twitter.com/PhilipLeftwich) - there is a strong R community on Twitter. If you want to join I can signpost some accounts to follow


## 1.3 Individual Exercises
### typing in commands in the R console
**Try these exercises on your own, and work at your own pace - the only goal is to have tried these before we meet on Friday - I estimate you will need about an hour to work through everything, but don't worry if it takes a little longer at first.**
**Make notes as you go, and write down any questions you have/ sections you don't understand - these can be added to our discussion board on Blackboard**
**Some questions will be signposted as for the Blackboard quiz, make sure you write these answers down so you can submit them later**

One of the easiest things you can do with R is use it as a simple calculator, so it's a good place to start. For instance, try typing `10 + 20`, and hitting enter.^[Seriously, **open up RStudio Cloud** and start typing - you should work in your own workspace for this - you will need to start up a project, but then just start typing into the console. The simple act of typing it rather than "just reading" makes a big difference. It makes the concepts more concrete, and it ties the abstract ideas (programming and statistics) to the actual context in which you need to use them. Statistics is something you *do*, not just something you read about in a textbook.] When you do this, you've entered a ***command***, and R will "execute" that command. What you see on screen now will be this:
...
Reminder the workshop runs on Friday afternoons (come to the right group for you!)  - you will need to join a Collaborate session and logon to RStudio Cloud^[Working with others, sharing problems & solutions with talking and code sharing is the fastest way to get better we will have a space to post problems and everyone should participate as much as they can. Not just in our 'live' sessions] is designed to be run in small groups, where you are encouraged to talk with each other and discuss your observations. However, all of the materials you need to complete this are available as an assignment on RStudio Cloud, so don't worry if you can't make the live session (or simply want to go over it again in your own time). 
